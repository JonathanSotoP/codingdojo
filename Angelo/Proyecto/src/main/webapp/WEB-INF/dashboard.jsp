	...
	<div class="container">
		<header>
			...
			<h1>Bienvenido a Tutti-Task,  <c:out value="${usuario.name}"/> <c:forEach items="${usuario.roles}"> <c:out value="${name}"/></c:forEach>  </h1>
			 <form style="display:flex;justify-content:flex-end;"id="logoutForm" method="POST" action="/logout">
		        ...
		        <input class="btn"  style="margin-right: 3rem; background-color: #a81f91;" type="submit" value="Logout!" />
	    	</form>

		</header>
			<hr>
		<a class="creartarea" href="/admin/crear">Crear tarea</a>
		<hr>
		...
		<table class="table">
				...
				<tbody>
					...
					<tr>
						<td scope="col"> <a id="mostrar"href="/admin/tareas/<c:out value="${tarea.id}"/>"> <c:out value="${tarea.nombre}"/> </a> </td>
						<td> <c:out value="${tarea.descripcion}"/> </td>
						<td> <c:out value="${tarea.tiempo}"/></td>
						<td style="display: flex;"> <a class="btn btn-primary" href="/admin/editar/<c:out value="${tarea.id}"/>" role="button">Editar</a> 
